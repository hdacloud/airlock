...
func runGetSlots(cmd *cobra.Command, cmdArgs []string) error {
	...
	for group, maxSlots := range runSettings.LockGroups {
		...
		manager, err := lock.NewManager(ctx, runSettings.EtcdEndpoints, runSettings.ClientCertPubPath, runSettings.ClientCertKeyPath, runSettings.EtcdUser, runSettings.EtcdPassword, runSettings.EtcdTxnTimeout, group, maxSlots)
		if err != nil {
			return err
		}
